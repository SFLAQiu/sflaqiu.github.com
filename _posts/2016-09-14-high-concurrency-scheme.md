...
对于更新频繁度不高，并且数据允许短时间内的延迟，可以通过数据静态化成JSON，XML,HTML等数据文件上传CND，在拉取数据的时候优先到CND拉取，如果没有获取到数据再从缓存，数据库中获取，当管理人员操作后台编辑数据再重新生成静态文件上传同步到CND，这样在高并发的时候可以使数据的获取命中在CND服务器上。
